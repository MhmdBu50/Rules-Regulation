...
public async Task<IActionResult> ViewPdf(int id)
{
    ...
    public async Task<IActionResult> GetActivityLogDetails(int logId)
    {
        ...
        catch (Exception ex)
        {
            ...
            if (await reader.ReadAsync())
            {
                _logger.LogInformation("Found activity log record for LogId: {LogId}", logId);

                // Get USER_ID (it's nullable in your schema)
                int? userId = reader.IsDBNull("USER_ID") ? null : reader.GetInt32("USER_ID");

                // Get ENTITY_ID (it's a NUMBER)
                int? entityId = reader.IsDBNull("ENTITY_ID") ? null : reader.GetInt32("ENTITY_ID");

                // Get TIMESTAMP
                DateTime? actionTimestamp = reader.IsDBNull("TIMESTAMP") ? null : reader.GetDateTime("TIMESTAMP");

                // Get CLOB data for BEFORE_DATA and AFTER_DATA
                ...
                {
                    using var beforeReader = reader.GetTextReader("BEFORE_DATA");
                    beforeData = await beforeReader.ReadToEndAsync();
                }

                if (!reader.IsDBNull("AFTER_DATA"))
                {
                    using var afterReader = reader.GetTextReader("AFTER_DATA");
                    afterData = await afterReader.ReadToEndAsync();
                }

                if (!reader.IsDBNull("DETAILS"))
                {
                    using var detailsReader = reader.GetTextReader("DETAILS");
                    details = await detailsReader.ReadToEndAsync();
                }
                ...
                var logEntry = new
                {
                    ...
                    UserId = userId,
                    UserName = reader.IsDBNull("USER_NAME") ? "Unknown" : reader.GetString("USER_NAME"),
                    UserRole = reader.IsDBNull("USER_ROLE") ? "Unknown" : reader.GetString("USER_ROLE"),
                    ActionType = reader.IsDBNull("ACTION_TYPE") ? "Unknown" : reader.GetString("ACTION_TYPE"),
                    EntityType = reader.IsDBNull("ENTITY_TYPE") ? "Unknown" : reader.GetString("ENTITY_TYPE"),
                    EntityId = entityId,
                    EntityName = reader.IsDBNull("ENTITY_NAME") ? "Unknown" : reader.GetString("ENTITY_NAME"),
                    ActionTimestamp = actionTimestamp?.ToString("yyyy-MM-dd HH:mm:ss") ?? "Unknown",
                    BeforeData = beforeData,
                    AfterData = afterData,
                    Details = details
                };

                _logger.LogInformation("Successfully created log entry object for LogId: {LogId}", logId);
                return Json(logEntry);
            }

            _logger.LogWarning("No activity log record found for LogId: {LogId}", logId);
            return NotFound();
        }
        catch (Exception ex)
        {
